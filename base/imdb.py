# -*- coding: utf-8 -*-
'''
Created on Tues July 31 10:58 2018
...
class imdb(object):
    def __init__(self, name):
        ...
        self.config = {} # Use this dict for storing dataset specific config options
    ...
    def get_widths(self):
        return [PIL.Image.open(self.image_path_at(i)).size[0] for i in range(self.nrof_images)]
        
    def get_heights(self):
        return [PIL.Image.open(self.image_path_at(i)).size[1] for i in range(self.nrof_images)]
        
    def get_sizes(self):
        return [PIL.Image.open(self.image_path_at(i)).size for i in range(self.nrof_images)]

    def append_flipped_images(self):
        nrof_images = self.nrof_images
    ...
    def create_roidb_from_box_list(self, box_list, gt_roidb):
        assert len(box_list) == self.nrof_images, 'Number of boxes must match number of ground-truth images'
        roidb = []
        for i in range(self.nrof_images):
            boxes = box_list[i]
            num_boxes = boxes.shape[0]
            overlaps = np.zeros((num_boxes, self.nrof_classes), dtype=np.float32)
    ...
    @staticmethod
    def filter_small_boxes(self, boxes, min_size):
        w = boxes[:, 2] - boxes[:, 0]
        h = boxes[:, 3] - boxes[:, 1]
