...
class pascal_voc(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        ...
        self._class_to_ind = dict(list(zip(self.classes, list(range(self.nrof_classes)))))
        self._image_ext = '.jpg'
    ...
    def _load_selective_search_roidb(self, gt_roidb):
        ...
        for i in range(raw_data.shape[0]):
            ...
            keep = self.unique_boxes(boxes)
            boxes = boxes[keep, :]
            keep = self.filter_small_boxes(boxes, self.config['min_size'])
            boxes = boxes[keep, :]
    ...
    def _load_pascal_annotation(self, index):
        ...
        boxes = np.zeros((nrof_objs, 4), dtype=np.uint16)
        gt_classes = np.zeros((nrof_objs), dtype=np.int32)
        overlaps = np.zeros((nrof_objs, self.nrof_classes), dtype=np.float32)
        seg_areas = np.zeros((nrof_objs), dtype=np.float32) # "Seg" area for pascal is just the box area
        ishards = np.zeros((nrof_objs), dtype=np.int32)
